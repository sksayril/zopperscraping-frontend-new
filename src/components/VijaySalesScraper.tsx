...
import { apiService, VijaySalesScrapeRequest, VijaySalesScrapeResponse, VijaySalesProductData } from '../services/api';
import VijaySalesProductDisplay from './VijaySalesProductDisplay';
import { validateVijaySalesUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function VijaySalesScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<VijaySalesProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<VijaySalesProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Vijay Sales product URL');
      return;
    }

    // Basic URL validation
    if (!validateVijaySalesUrl(url)) {
      setError('Please enter a valid Vijay Sales product URL (must contain vijaysales.com and /p/)');
      return;
    }
    ...
    try {
      const request: VijaySalesScrapeRequest = { url: url.trim() };
      const response: VijaySalesScrapeResponse = await apiService.scrapeVijaySalesProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Vijay Sales Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="vijaysales-url" className="block text-sm font-medium text-gray-700 mb-2">
              Vijay Sales Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.vijaysales.com/p/P234571/234571/product-name"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Vijay Sales product URL to scrape product details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <VijaySalesProductDisplay product={scrapedProduct} />
      )}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product, index) => (
              <div
                key={`${product.productUrl}-${product.scrapedAt}`}
