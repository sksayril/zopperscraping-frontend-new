...
import { apiService, PepperfryScrapeRequest, PepperfryScrapeResponse, PepperfryProductData } from '../services/api';
import PepperfryProductDisplay from './PepperfryProductDisplay';
import { validatePepperfryUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function PepperfryScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<PepperfryProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<PepperfryProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Pepperfry product URL');
      return;
    }

    // Basic URL validation
    if (!validatePepperfryUrl(url)) {
      setError('Please enter a valid Pepperfry product URL (must contain pepperfry.com/product/ and .html)');
      return;
    }
    ...
    try {
      const request: PepperfryScrapeRequest = { url: url.trim() };
      const response: PepperfryScrapeResponse = await apiService.scrapePepperfryProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Pepperfry Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="pepperfry-url" className="block text-sm font-medium text-gray-700 mb-2">
              Pepperfry Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.pepperfry.com/product/product-name-123456.html"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Pepperfry product URL to scrape furniture details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <PepperfryProductDisplay product={scrapedProduct} />
      )}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images[8] || product.product.images[0]} // Use product image, not logo
