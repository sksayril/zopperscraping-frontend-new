...
import { apiService, ChromaScrapeRequest, ChromaScrapeResponse, ChromaProductData } from '../services/api';
import ChromaProductDisplay from './ChromaProductDisplay';
import { validateChromaUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function ChromaScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<ChromaProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<ChromaProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Chroma product URL');
      return;
    }

    // Basic URL validation
    if (!validateChromaUrl(url)) {
      setError('Please enter a valid Chroma product URL (must contain croma.com and /p/)');
      return;
    }
    ...
    try {
      const request: ChromaScrapeRequest = { url: url.trim() };
      const response: ChromaScrapeResponse = await apiService.scrapeChromaProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Chroma Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="chroma-url" className="block text-sm font-medium text-gray-700 mb-2">
              Chroma Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.croma.com/product-name/p/product-id"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Chroma product URL to scrape product details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <ChromaProductDisplay product={scrapedProduct} />
      )}
