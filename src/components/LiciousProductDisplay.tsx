...
import { ExternalLink, Tag, Package, Truck, Shield, Heart, Share2, Download, ChevronDown, ChevronUp, Check, X } from 'lucide-react';
import { LiciousProductData } from '../services/api';
import { formatDate, generateImagePlaceholder } from '../utils/helpers';

interface Props { product: LiciousProductData; }

export default function LiciousProductDisplay({ product }: Props) {
  ...
  const productImages = data.images.filter(img => img.includes('cdn.licious.in') || img.includes('assets.licious.in'));
  ...
  const handleDownloadData = () => {
    ...
    link.href = objUrl;
    link.download = `licious-product-${data.name.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.json`;
      ...
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Licious Product Details</h1>
              <p className="text-purple-100 text-sm">Scraped on {formatDate(scrapedAt)}</p>
            </div>
          </div>
            ...
            {productImages.length > 1 && (
              <div className="grid grid-cols-4 gap-2">
                {productImages.slice(0, 8).map((image, index) => (
                  <button key={index} onClick={() => setSelectedImageIndex(index)} className={`aspect-square rounded-lg overflow-hidden border-2 ${selectedImageIndex === index ? 'border-purple-500 ring-2 ring-purple-200' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-gray-600">Source:</span><span className="text-gray-900">{source}</span></div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {expandedSections.features && (
                <div className="mt-3 space-y-4">
                  {data.features.included && data.features.included.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {data.features.included.map((feature, index) => (
                  ...
                  {data.features.excluded && data.features.excluded.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">
                        {data.features.excluded.map((feature, index) => (
                          <li key={index} className="text-sm text-gray-700 flex items-start">
                            <X className="w-3 h-3 text-red-600 mr-2 mt-0.5 flex-shrink-0" />
                            {feature}
                          </li>
                        ))}
                      </ul>
