...
import DMartProductDisplay from './DMartProductDisplay';
import { 
  validateDMartUrl, 
  formatDate, 
...
export default function DMartScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<DMartProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<DMartProductData[]>([]);

  const handleScrape = async () => {
    const sanitizedUrl = sanitizeInput(url);
    if (!sanitizedUrl) {
      setError('Please enter a valid DMart product URL');
      return;
    }
    ...
    if (!validateDMartUrl(sanitizedUrl)) {
      setError('Please enter a valid DMart product URL (must contain dmart.in and /product/)');
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      const request: DMartScrapeRequest = { url: sanitizedUrl };
      const response: DMartScrapeResponse = await apiService.scrapeDMartProduct(request);
      ...
      logError(err, 'DMartScraper.handleScrape');
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">DMart Product Scraper</h2>
        </div>
        <div className="space-y-4">
          <div>
            <label htmlFor="dmart-url" className="block text-sm font-medium text-gray-700 mb-2">DMart Product URL</label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.dmart.in/product/PRODUCT-NAME"
            ...
            <p className="mt-2 text-sm text-gray-500">Enter a valid DMart product URL to scrape product details, images, variants, and pricing information.</p>
          </div>
      ...
      {scrapedProduct && (<DMartProductDisplay product={scrapedProduct} />)}
            ...
            {scrapeHistory.map((product) => (
              <div key={`${product.url}-${product.scrapedAt}`} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-orange-600">₹{product.product.sellingPrice}</span>
