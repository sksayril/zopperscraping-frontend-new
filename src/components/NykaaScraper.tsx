...
import { apiService, NykaaScrapeRequest, NykaaScrapeResponse, NykaaProductData } from '../services/api';
import NykaaProductDisplay from './NykaaProductDisplay';
import { validateNykaaUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function NykaaScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<NykaaProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<NykaaProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Nykaa product URL');
      return;
    }

    // Basic URL validation
    if (!validateNykaaUrl(url)) {
      setError('Please enter a valid Nykaa product URL (must contain nykaa.com and /p/)');
      return;
    }
    ...
    try {
      const request: NykaaScrapeRequest = { url: url.trim() };
      const response: NykaaScrapeResponse = await apiService.scrapeNykaaProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Nykaa Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="nykaa-url" className="block text-sm font-medium text-gray-700 mb-2">
              Nykaa Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.nykaa.com/product-name/p/product-id"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Nykaa product URL to scrape product details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <NykaaProductDisplay product={scrapedProduct} />
      )}
