...
import { apiService, UrbanicScrapeRequest, UrbanicScrapeResponse, UrbanicProductData } from '../services/api';
import UrbanicProductDisplay from './UrbanicProductDisplay';
import { validateUrbanicUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function UrbanicScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<UrbanicProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<UrbanicProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Urbanic product URL');
      return;
    }

    // Basic URL validation
    if (!validateUrbanicUrl(url)) {
      setError('Please enter a valid Urbanic product URL (must contain urbanic.com and /details/)');
      return;
    }
    ...
    try {
      const request: UrbanicScrapeRequest = { url: url.trim() };
      const response: UrbanicScrapeResponse = await apiService.scrapeUrbanicProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Urbanic Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="urbanic-url" className="block text-sm font-medium text-gray-700 mb-2">
              Urbanic Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://in.urbanic.com/details/product-name-1234567?vid=308&skcPGs=257741-356277"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Urbanic product URL to scrape fashion details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <UrbanicProductDisplay product={scrapedProduct} />
      )}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images[2] || product.product.images[0]} // Use product image, not UI elements
