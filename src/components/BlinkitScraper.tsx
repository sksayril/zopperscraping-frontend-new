...
import { apiService, BlinkitScrapeRequest, BlinkitScrapeResponse, BlinkitProductData } from '../services/api';
import BlinkitProductDisplay from './BlinkitProductDisplay';
import { validateBlinkitUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function BlinkitScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<BlinkitProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<BlinkitProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Blinkit product URL');
      return;
    }

    // Basic URL validation
    if (!validateBlinkitUrl(url)) {
      setError('Please enter a valid Blinkit product URL (must contain blinkit.com, /prn/, and /prid/)');
      return;
    }
    ...
    try {
      const request: BlinkitScrapeRequest = { url: url.trim() };
      const response: BlinkitScrapeResponse = await apiService.scrapeBlinkitProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Blinkit Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="blinkit-url" className="block text-sm font-medium text-gray-700 mb-2">
              Blinkit Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://blinkit.com/prn/product-name/prid/12345"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Blinkit product URL to scrape grocery details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <BlinkitProductDisplay product={scrapedProduct} />
      )}
