...
import { apiService, ZeptoScrapeRequest, ZeptoScrapeResponse, ZeptoProductData } from '../services/api';
import ZeptoProductDisplay from './ZeptoProductDisplay';
import { validateZeptoUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function ZeptoScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<ZeptoProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<ZeptoProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Zepto product URL');
      return;
    }

    // Basic URL validation
    if (!validateZeptoUrl(url)) {
      setError('Please enter a valid Zepto product URL (must contain zeptonow.com/pn/ and /pvid/)');
      return;
    }
    ...
    try {
      const request: ZeptoScrapeRequest = { url: url.trim() };
      const response: ZeptoScrapeResponse = await apiService.scrapeZeptoProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Zepto Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="zepto-url" className="block text-sm font-medium text-gray-700 mb-2">
              Zepto Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.zeptonow.com/pn/product-name/pvid/abc123..."
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Zepto product URL to scrape product details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <ZeptoProductDisplay product={scrapedProduct} />
      )}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images[2] || product.product.images[0]} // Use product image, not logo
