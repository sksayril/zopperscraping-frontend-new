...
import { 
  apiService, 
  LiciousScrapeRequest, 
  LiciousScrapeResponse, 
  LiciousProductData 
} from '../services/api';
import LiciousProductDisplay from './LiciousProductDisplay';
import { 
  validateLiciousUrl, 
  formatDate, 
...
export default function LiciousScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<LiciousProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<LiciousProductData[]>([]);

  const handleScrape = async () => {
    const sanitizedUrl = sanitizeInput(url);
    if (!sanitizedUrl) {
      setError('Please enter a valid Licious product URL');
      return;
    }
    ...
    if (!validateLiciousUrl(sanitizedUrl)) {
      setError('Please enter a valid Licious product URL (must contain licious.in and /seafood/, /meat/, or /chicken/)');
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      const request: LiciousScrapeRequest = { url: sanitizedUrl };
      const response: LiciousScrapeResponse = await apiService.scrapeLiciousProduct(request);
      ...
      logError(err, 'LiciousScraper.handleScrape');
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Licious Product Scraper</h2>
        </div>
        <div className="space-y-4">
          <div>
            <label htmlFor="licious-url" className="block text-sm font-medium text-gray-700 mb-2">Licious Product URL</label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.licious.in/seafood/PRODUCT-NAME"
              ...
              <button
                ...
                className="px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center space-x-2"
              >
                {isLoading ? (<><Loader2 className="w-4 h-4 animate-spin" /><span>Scraping...</span></>) : (<><Search className="w-4 h-4" /><span>Scrape</span></>)}
              </button>
            </div>
            <p className="mt-2 text-sm text-gray-500">Enter a valid Licious product URL to scrape fresh meat, seafood, and chicken product details, images, and pricing information.</p>
          </div>
      ...
      {scrapedProduct && (<LiciousProductDisplay product={scrapedProduct} />)}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              <div key={`${product.url}-${product.scrapedAt}`} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images.find(img => img.includes('cdn.licious.in')) || product.product.images[0]}
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-purple-600">₹{product.product.sellingPrice}</span>
