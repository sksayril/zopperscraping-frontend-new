...
import { apiService, OneMgScrapeRequest, OneMgScrapeResponse, OneMgProductData } from '../services/api';
import OneMgProductDisplay from './OneMgProductDisplay';
import { validateOneMgUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function OneMgScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<OneMgProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<OneMgProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid 1mg product URL');
      return;
    }

    // Basic URL validation
    if (!validateOneMgUrl(url)) {
      setError('Please enter a valid 1mg product URL (must contain 1mg.com and /otc/ or /p/)');
      return;
    }
    ...
    try {
      const request: OneMgScrapeRequest = { url: url.trim() };
      const response: OneMgScrapeResponse = await apiService.scrapeOneMgProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">1mg Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="1mg-url" className="block text-sm font-medium text-gray-700 mb-2">
              1mg Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.1mg.com/otc/product-name-otc123456"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid 1mg product URL to scrape product details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <OneMgProductDisplay product={scrapedProduct} />
      )}
