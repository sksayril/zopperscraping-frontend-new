...
import { 
  apiService, 
  ApolloPharmacyScrapeRequest, 
  ApolloPharmacyScrapeResponse, 
  ApolloPharmacyProductData 
} from '../services/api';
import ApolloPharmacyProductDisplay from './ApolloPharmacyProductDisplay';
import { 
  validateApolloPharmacyUrl, 
  formatDate, 
...
export default function ApolloPharmacyScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<ApolloPharmacyProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<ApolloPharmacyProductData[]>([]);

  const handleScrape = async () => {
    const sanitizedUrl = sanitizeInput(url);
    if (!sanitizedUrl) {
      setError('Please enter a valid Apollo Pharmacy product URL');
      return;
    }
    ...
    if (!validateApolloPharmacyUrl(sanitizedUrl)) {
      setError('Please enter a valid Apollo Pharmacy product URL (must contain apollopharmacy.in and /otc/ or /medicine/)');
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      const request: ApolloPharmacyScrapeRequest = { url: sanitizedUrl };
      const response: ApolloPharmacyScrapeResponse = await apiService.scrapeApolloPharmacyProduct(request);
      ...
      logError(err, 'ApolloPharmacyScraper.handleScrape');
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Apollo Pharmacy Product Scraper</h2>
        </div>
        <div className="space-y-4">
          <div>
            <label htmlFor="apollo-pharmacy-url" className="block text-sm font-medium text-gray-700 mb-2">Apollo Pharmacy Product URL</label>
            <div className="flex space-x-3">
              <input
                id="apollo-pharmacy-url"
                type="url"
                value={url}
                onChange={handleUrlChange}
                onKeyPress={handleKeyPress}
                placeholder="https://www.apollopharmacy.in/otc/PRODUCT-NAME"
            ...
            <p className="mt-2 text-sm text-gray-500">Enter a valid Apollo Pharmacy product URL to scrape product details, images, and pricing information.</p>
          </div>
      ...
      {scrapedProduct && (<ApolloPharmacyProductDisplay product={scrapedProduct} />)}
            ...
            {scrapeHistory.map((product) => (
              <div key={`${product.url}-${product.scrapedAt}`} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-blue-600">₹{product.product.sellingPrice}</span>
