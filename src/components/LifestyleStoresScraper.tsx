...
import { 
  apiService, 
  LifestyleStoresScrapeRequest, 
  LifestyleStoresScrapeResponse, 
  LifestyleStoresProductData 
} from '../services/api';
import LifestyleStoresProductDisplay from './LifestyleStoresProductDisplay';
import { 
  validateLifestyleStoresUrl, 
  formatDate, 
...
export default function LifestyleStoresScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<LifestyleStoresProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<LifestyleStoresProductData[]>([]);

  const handleScrape = async () => {
    const sanitizedUrl = sanitizeInput(url);
    
    if (!sanitizedUrl) {
      setError('Please enter a valid Lifestyle Stores product URL');
      return;
    }
    ...
    if (!validateLifestyleStoresUrl(sanitizedUrl)) {
      setError('Please enter a valid Lifestyle Stores product URL (must contain lifestylestores.com and /p/)');
      return;
    }
    ...
    try {
      const request: LifestyleStoresScrapeRequest = { url: sanitizedUrl };
      const response: LifestyleStoresScrapeResponse = await apiService.scrapeLifestyleStoresProduct(request);
      ...
      logError(err, 'LifestyleStoresScraper.handleScrape');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Lifestyle Stores Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="lifestylestores-url" className="block text-sm font-medium text-gray-700 mb-2">
              Lifestyle Stores Product URL
            </label>
            <div className="flex space-x-3">
              <input
                id="lifestylestores-url"
                type="url"
                value={url}
                onChange={handleUrlChange}
                onKeyPress={handleKeyPress}
                placeholder="https://www.lifestylestores.com/in/en/product-name/p/PRODUCTCODE"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Lifestyle Stores product URL to scrape fashion and lifestyle product details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <LifestyleStoresProductDisplay product={scrapedProduct} />
      )}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images[4] || product.product.images[0]} // Use product image, not banner images
