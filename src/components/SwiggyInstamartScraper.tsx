...
import { apiService, SwiggyInstamartScrapeRequest, SwiggyInstamartScrapeResponse, SwiggyInstamartProductData } from '../services/api';
import SwiggyInstamartProductDisplay from './SwiggyInstamartProductDisplay';
import { validateSwiggyInstamartUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function SwiggyInstamartScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<SwiggyInstamartProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<SwiggyInstamartProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Swiggy Instamart product URL');
      return;
    }

    // Basic URL validation
    if (!validateSwiggyInstamartUrl(url)) {
      setError('Please enter a valid Swiggy Instamart product URL (must contain swiggy.com/instamart/item/ and storeId=)');
      return;
    }
    ...
    try {
      const request: SwiggyInstamartScrapeRequest = { url: url.trim() };
      const response: SwiggyInstamartScrapeResponse = await apiService.scrapeSwiggyInstamartProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Swiggy Instamart Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="swiggy-instamart-url" className="block text-sm font-medium text-gray-700 mb-2">
              Swiggy Instamart Product URL
            </label>
            <div className="flex space-x-3">
              <input
                id="swiggy-instamart-url"
                type="url"
                value={url}
                onChange={(e) => setUrl(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder="https://www.swiggy.com/instamart/item/ABC123?storeId=123456"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Swiggy Instamart product URL to scrape grocery details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <SwiggyInstamartProductDisplay product={scrapedProduct} />
      )}
