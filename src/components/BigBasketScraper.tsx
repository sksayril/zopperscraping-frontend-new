...
import { apiService, BigBasketScrapeRequest, BigBasketScrapeResponse, BigBasketProductData } from '../services/api';
import BigBasketProductDisplay from './BigBasketProductDisplay';
import { validateBigBasketUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function BigBasketScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<BigBasketProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<BigBasketProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid BigBasket product URL');
      return;
    }

    // Basic URL validation
    if (!validateBigBasketUrl(url)) {
      setError('Please enter a valid BigBasket product URL (must contain bigbasket.com/pd/)');
      return;
    }
    ...
    try {
      const request: BigBasketScrapeRequest = { url: url.trim() };
      const response: BigBasketScrapeResponse = await apiService.scrapeBigBasketProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">BigBasket Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="bigbasket-url" className="block text-sm font-medium text-gray-700 mb-2">
            ...
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.bigbasket.com/pd/10000067/product-name/"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid BigBasket product URL to scrape grocery details, images, and pricing information.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <BigBasketProductDisplay product={scrapedProduct} />
      )}
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-green-600">
                    ₹{product.product.discount.prim_price.sp}
                  </span>
                  <button
