...
import { apiService, AjioScrapeRequest, AjioScrapeResponse, AjioProductData } from '../services/api';
import AjioProductDisplay from './AjioProductDisplay';
import { validateAjioUrl, formatPrice, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function AjioScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<AjioProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<AjioProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Ajio product URL');
      return;
    }

    // Basic URL validation
    if (!validateAjioUrl(url)) {
      setError('Please enter a valid Ajio product URL (must contain ajio.com and /p/)');
      return;
    }
    ...
    try {
      const request: AjioScrapeRequest = { url: url.trim() };
      const response: AjioScrapeResponse = await apiService.scrapeAjioProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Ajio Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="ajio-url" className="block text-sm font-medium text-gray-700 mb-2">
              Ajio Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.ajio.com/product-name/p/product-id"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Ajio product URL to scrape product details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <AjioProductDisplay product={scrapedProduct} />
      )}
