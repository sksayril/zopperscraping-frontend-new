...
import { apiService, PharmEasyScrapeRequest, PharmEasyScrapeResponse, PharmEasyProductData } from '../services/api';
import PharmEasyProductDisplay from './PharmEasyProductDisplay';
import { validatePharmEasyUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function PharmEasyScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<PharmEasyProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<PharmEasyProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid PharmEasy product URL');
      return;
    }

    // Basic URL validation
    if (!validatePharmEasyUrl(url)) {
      setError('Please enter a valid PharmEasy product URL (must contain pharmeasy.in and /online-medicine-order/)');
      return;
    }
    ...
    try {
      const request: PharmEasyScrapeRequest = { url: url.trim() };
      const response: PharmEasyScrapeResponse = await apiService.scrapePharmEasyProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">PharmEasy Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="pharmeasy-url" className="block text-sm font-medium text-gray-700 mb-2">
              PharmEasy Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://pharmeasy.in/online-medicine-order/product-name-12345"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid PharmEasy product URL to scrape medicine details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <PharmEasyProductDisplay product={scrapedProduct} />
      )}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images.find(img => img.includes('cdn01.pharmeasy.in/dam/products')) || product.product.images[0]}
