...
import { DMartProductData } from '../services/api';
import { formatDate, generateImagePlaceholder } from '../utils/helpers';

interface Props { product: DMartProductData; }

export default function DMartProductDisplay({ product }: Props) {
  ...
  const productImages = data.images.filter(img => img.includes('/images/products/'));
  ...
  const handleDownloadData = () => {
    ...
    link.href = objUrl;
    link.download = `dmart-product-${data.brand.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.json`;
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      <div className="bg-gradient-to-r from-orange-600 to-orange-700 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">DMart Product Details</h1>
              <p className="text-orange-100 text-sm">Scraped on {formatDate(scrapedAt)}</p>
            </div>
          </div>
      ...
      <div className="p-6">
            ...
            {productImages.length > 1 && (
              <div className="grid grid-cols-4 gap-2">
                {productImages.slice(0, 8).map((image, index) => (
                  <button key={index} onClick={() => setSelectedImageIndex(index)} className={`aspect-square rounded-lg overflow-hidden border-2 ${selectedImageIndex === index ? 'border-orange-500 ring-2 ring-orange-200' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-600">Brand:</span><span className="text-gray-900">{data.brand}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Discount:</span><span className="text-gray-900">₹{data.discount.toLocaleString()}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Discount %:</span><span className="text-gray-900">{data.discountPercent}%</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Total Images:</span><span className="text-gray-900">{data.images.length}</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-6">
          {data.description && (
            <div className="bg-gray-50 rounded-lg p-4">
              <button onClick={() => toggleSection('description')} className="flex items-center justify-between w-full text-left">
          ...
          {data.variants.length > 0 && (
            <div className="bg-gray-50 rounded-lg p-4">
              <button onClick={() => toggleSection('variants')} className="flex items-center justify-between w-full text-left">
              ...
              {expandedSections.variants && (
                <div className="mt-3 space-y-3">
                  {data.variants.map((variant, index) => (
                    <div key={index} className={`p-3 rounded-lg border ${variant.isSelected ? 'border-orange-500 bg-orange-50' : 'border-gray-200 bg-white'}`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium text-gray-900">{variant.volume}</p>
                          <p className="text-sm text-gray-600">{variant.pricePerUnit}</p>
                        </div>
                        {variant.isSelected && (
                          <span className="bg-orange-100 text-orange-800 px-2 py-1 rounded-full text-xs font-semibold">Selected</span>
                        )}
                      </div>
                    </div>
                  ))}
