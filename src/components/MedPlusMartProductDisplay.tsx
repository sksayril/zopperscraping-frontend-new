...
import { MedPlusMartProductData } from '../services/api';
import { formatDate, generateImagePlaceholder } from '../utils/helpers';

interface Props {
  product: MedPlusMartProductData;
}

export default function MedPlusMartProductDisplay({ product }: Props) {
  const [selectedImageIndex, setSelectedImageIndex] = useState(0);
  // reserved for future use when we paginate details
  ...
  // Filter to prefer product images over banner/icons
  const productImages = data.images.filter((img, idx) => idx >= 9 && !img.includes('banner') && !img.includes('assets'));
  ...
  const handleDownloadData = () => {
    ...
    link.href = objUrl;
    link.download = `medplusmart-product-${data.productDetails.manufacturer}-${new Date().toISOString().split('T')[0]}.json`;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-teal-100 text-sm">Scraped on {formatDate(scrapedAt)}</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            <button onClick={() => setIsWishlisted(!isWishlisted)} className={`p-2 rounded-lg ${isWishlisted ? 'bg-red-500 text-white' : 'bg-white/20 text-white hover:bg-white/30'}`}> <Heart className={`w-5 h-5 ${isWishlisted ? 'fill-current' : ''}`} /> </button>
            ...
            {productImages.length > 1 && (
              <div className="grid grid-cols-4 gap-2">
                {productImages.slice(0, 8).map((image, index) => (
                  <button key={index} onClick={() => setSelectedImageIndex(index)} className={`aspect-square rounded-lg overflow-hidden border-2 ${selectedImageIndex === index ? 'border-teal-500 ring-2 ring-teal-200' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                {data.productDetails.composition && (
                  <div className="flex justify-between"><span className="text-gray-600">Composition:</span><span className="text-gray-900 text-right">{data.productDetails.composition}</span></div>
                )}
              </div>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {data.variants.map((variant, idx) => (
                    <span key={idx} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm">{variant}</span>
                  ))}
                </div>
              </div>
            )}
