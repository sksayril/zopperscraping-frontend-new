...
import { apiService, NetmedsScrapeRequest, NetmedsScrapeResponse, NetmedsProductData } from '../services/api';
import NetmedsProductDisplay from './NetmedsProductDisplay';
import { validateNetmedsUrl, formatDate, generateImagePlaceholder } from '../utils/helpers';

export default function NetmedsScraper() {
  const [url, setUrl] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [scrapedProduct, setScrapedProduct] = useState<NetmedsProductData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [scrapeHistory, setScrapeHistory] = useState<NetmedsProductData[]>([]);

  const handleScrape = async () => {
    if (!url.trim()) {
      setError('Please enter a valid Netmeds product URL');
      return;
    }

    // Basic URL validation
    if (!validateNetmedsUrl(url)) {
      setError('Please enter a valid Netmeds product URL (must contain netmeds.com and /product/)');
      return;
    }
    ...
    try {
      const request: NetmedsScrapeRequest = { url: url.trim() };
      const response: NetmedsScrapeResponse = await apiService.scrapeNetmedsProduct(request);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Netmeds Product Scraper</h2>
        </div>
        
        <div className="space-y-4">
          {/* URL Input */}
          <div>
            <label htmlFor="netmeds-url" className="block text-sm font-medium text-gray-700 mb-2">
              Netmeds Product URL
            </label>
            <div className="flex space-x-3">
              <input
                ...
                placeholder="https://www.netmeds.com/product/product-name-12345"
            ...
            <p className="mt-2 text-sm text-gray-500">
              Enter a valid Netmeds product URL to scrape medicine details, images, and specifications.
            </p>
          </div>
      ...
      {scrapedProduct && (
        <NetmedsProductDisplay product={scrapedProduct} />
      )}
          ...
          <div className="space-y-3">
            {scrapeHistory.map((product) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={product.product.images.find(img => img.includes('netmed/wrkr/products/assets/item')) || product.product.images[0]}
